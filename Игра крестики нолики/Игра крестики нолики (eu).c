...
int main(){
    ...
    int cross = 1, zero = 0;
    printf("\n  Help: It is recommended to use the side keys on the right for playing\n");
    printf("\n  Help: Fields under numbers from %c to %c (inclusive) are considered free\n", box1, box9);
    int game_number = -1;
    printf("\n  Pick what you want to play with, if it's a cross, click %d, if it's a zero, press %d, (then press the key enter)\n\n  ", cross, zero);
    scanf("%d", &game_number);
    if(game_number != cross and game_number != zero){
        printf("\n  You should have entered %d or %d\n", cross, zero);
        abort();
    }
...
int game(char player, char bot, char box1, char box2, char box3, char box4, char box5, char box6, char box7, char box8, char box9, int i, int num){
    ...
    if(scanf("%d", &num) != 1){
        printf("\n  You had to pick a free cell, team won %c\n", bot);
        return 0;
    }
    if(1 == num){
